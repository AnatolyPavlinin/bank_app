...
def get_transactions_file_csv(file_csv: Union[str, Path]) -> List[Dict[Hashable, Any]]:
    """Функция для считывания фининасовых операций из CSV. Возвращает список словарей с транзакцией"""

    try:
        with open(file_csv, encoding="utf-8") as file:
            csv_transaction = pd.read_csv(file, delimiter=";")
            transactions = csv_transaction.to_dict("records")
            return transactions
